...
class UsuariosDesactivadosFragment : Fragment(), UsuariosAdaptador.UsuarioOnItemClickListener{

    private lateinit var token:String
    private lateinit var usuario: Usuario
    private var listaUsuariosDesactivados: ArrayList<Usuario> =ArrayList<Usuario>()
    ...
    override fun OnItemClick(posicion: Int) {
        ...
        builder.setTitle("Activar Usuario")
        builder.setMessage("¿Está seguro de activar el Usuario?")
        builder.setPositiveButton("OK",
            DialogInterface.OnClickListener { dialog, which ->
                val usuario=listaUsuariosDesactivados.get(posicion)
                usuario.activo=true
    ...
    private fun asynTaskObtenerListadoTalleres(){
        ...
        try{
            ...
            Toast.makeText(context, "Ha ocurrido un error al obtener la Lista de Usuarios Desactivados", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun servicioObtenerUsuariosActivos(): List<Usuario>?{
        ...
        try{
            ...
            val call =servicioRegistroAsistencia.obtenerUsuariosDesactivados( "Bearer $token")
    ...
    /*****************************************************/
    private fun activarUsuario(usuario: Usuario){

        try{
            val servicio = ClienteApiRest.getRetrofitInstance().create(UsuarioServicio::class.java)
            val call = servicio.activarUsuario(usuario, "Bearer " + token)

            call.enqueue(object : Callback<Usuario>{

                override fun onFailure(call: Call<Usuario>, t: Throwable) {
                    Toast.makeText(context, "Ha ocurrido un error al activar el usuario", Toast.LENGTH_SHORT).show()
                }
                    ...
                    if(response != null){
                        ...
                        if(response.code()==200){
                            ...
                            if(usuarioDesactivado != null){
                                ...
                                listaUsuariosDesactivados.remove(usuario)

                                adaptador.notifyDataSetChanged()

                                Toast.makeText(context, "Se ha activado el usuario correctamente", Toast.LENGTH_SHORT).show()
    ...
    private fun asignarListaUsuariosRecyclerView(listaUsuariosActivosAux: List<Usuario>){
        ...
        if(listaUsuariosActivosAux.size > 0){
            ...
            listaUsuariosDesactivados= listaUsuariosActivosAux as ArrayList<Usuario>
            adaptador = UsuariosAdaptador(listaUsuariosActivosAux, this@UsuariosDesactivadosFragment)
