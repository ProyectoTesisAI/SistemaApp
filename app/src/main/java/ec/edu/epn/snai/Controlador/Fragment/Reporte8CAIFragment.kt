...
class Reporte8CAIFragment:Fragment() {
    ...
    private fun obtenerListaReporte8S(){
        try{
            ...
            val call = servicio.obtenerEdadReporteNivelEducativoSiCAI(edadSBusqueda,"Bearer " + token)
    ...
    private fun obtenerListaReporte8N(){
        try{
            val servicio = ClienteApiRest.getRetrofitInstance().create(ReporteServicio::class.java)
            val call = servicio.obtenerEdadReporteNivelEducativoNoCAI(edadNBusqueda,"Bearer " + token)
