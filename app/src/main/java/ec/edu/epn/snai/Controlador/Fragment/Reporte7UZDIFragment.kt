...
class Reporte7UZDIFragment:Fragment() {
    ...
    private fun obtenerListaReporte6S(nivel: String){
        try{
            ...
            val call = servicio.obtenerReporteNivelEducativoSUZDI(nivelBusqueda,"Bearer " + token)
    ...
    private fun obtenerListaReporte6N(nivel: String){
        try{
            val servicio = ClienteApiRest.getRetrofitInstance().create(ReporteServicio::class.java)
            val call = servicio.obtenerReporteNivelEducativoNUZDI(nivel,"Bearer " + token)
